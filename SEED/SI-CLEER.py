...
def read_data_path(path, expNo):
    all_files_path=glob(path)
    negative_file_path = []
    positive_file_path = []
    normal_file_path = []
    exp_trial_scope = np.arange(expNo*15+1,(expNo+1)*15+1, 1)
    for i in all_files_path:
        if int(i.split('/')[4].split('_')[0]) in exp_trial_scope:
            if  '-1' == (i.split('/')[4]).split('_')[3]: # 匹配指定路径下的所有.fif文件，负性的trial数据:
                negative_file_path.append(i)
            elif '1' == (i.split('/')[4]).split('_')[3]:
                positive_file_path.append(i)
            elif '0' == (i.split('/')[4]).split('_')[3]:
                normal_file_path.append(i)
            else:
                continue
        else:
            continue

    return negative_file_path,positive_file_path,normal_file_path

# 定义读取数据函数
def read_data(file_path):
    data=mne.read_epochs(file_path,preload=True) #seed数据集的采样率是200
...
if __name__ == '__main__':
    ...
    exps_accs = []
    for expNo in range(45):
        print('validation data from experiment =============================================== {}'.format(expNo+1))
        #2. 读取每类trial数据的路径，15个用户，参加3次实验，每次实验15个trial,分正、中、负三类性质，共675个trial
        neg_file_path,pos_file_path,norm_file_path = read_data_path('../data/Preprocessed_CL/SEED/*.fif', expNo)
        #-----------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        norm_epochs_array=[read_data(trial) for trial in norm_file_path]
        ...
        norm_epoch_labels=[len(i)*[2] for i in norm_epochs_array] #中性trial的各个epoch的标签列表

        #-----------------------------------------------------------------------------------------------------------------------------------------------------------

        #5. 合并有病和健康的数据、标签
        data_list=neg_epochs_array + pos_epochs_array + norm_epochs_array
        label_list=neg_epochs_labels + pos_epochs_labels + norm_epoch_labels
        np.save('SEED_data_list_exp_{}_3class.npy'.format(expNo+1), data_list)
        np.save('SEED_label_list_exp_{}_3class.npy'.format(expNo+1), label_list)
        ...
        #for train_index, val_index in sgkf.split(exp_data_array, exp_label_array, exp_group_array):
            ...
            norm_indices = np.where(exp_label_array == 2)[0]
            ...
            train_norm_indices = np.intersect1d(norm_indices, train_index)
            test_neg_indices = np.intersect1d(neg_indices, val_index)
            test_pos_indices = np.intersect1d(pos_indices, val_index)
            test_norm_indices = np.intersect1d(norm_indices, val_index)

            # 8.4 将neg和pos样本索引合并，形成训练集和测试集的索引
            train_indices = np.concatenate((train_neg_indices, train_pos_indices, train_norm_indices))
            test_indices = np.concatenate((test_neg_indices, test_pos_indices, test_norm_indices))
    ...
    np.save('exps_accs_3class.npy', exps_accs)